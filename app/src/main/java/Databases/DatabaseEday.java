...
public class DatabaseEday extends SQLiteOpenHelper {
    ...
    public static final String DatabaseName = "DatabaseEday.db";
    public static final String TableName = "TableE";
    ...
    public int getTotal(String course, String series, String section, String roll, String cycles,String day) {
        SQLiteDatabase db = this.getWritableDatabase();
        int count=0;
            String SQL = "SELECT State,Count FROM " + TableName +" WHERE ID = " + "'" + course+series+section+preference.getNumber()+"'"+" AND Cycle= " + "'" + cycles+ "'"+ " AND Day='"+day+"'"+" AND Roll='"+roll+"'";
            Cursor cursor = db.rawQuery(SQL, null);
            while (cursor.moveToNext()) {
                if (cursor.getString(0).equals("true") && cursor.getString(1).equals("1")){
                    count++;
                }
            }
